...
| **[`playwright-web-qa`](plugins/playwright-web-qa/skills/playwright-web-qa/SKILL.md)** | explicit testing requests only — "test / QA / check this page or flow in the browser", "check the UX", "check alignment", "boy scout", or `/web-qa`; deliberately **not** triggered while planning or implementing a feature, so QA stays a separate phase after development | Browser-driven exploratory QA — snapshot-first Playwright MCP mechanics, console + network after every step, user-flow walking in all variants (happy / alternate / error / abandon-and-resume / first-time / per-role), per-screen functional heuristics, accessibility via the a11y tree, Nielsen-based UX evaluation with 0–4 severity, measured layout quality (alignment, spacing rhythm, form composition and field balance, typography), Boy Scout mode that fixes findings at the root and loops until clean, structured QA report |
...
- **Test cases and run records in `docs/qa/`** — test cases are Markdown files with YAML front matter, one per case, grouped by flow. Each carries its variant, technique, priority, oracle, origin, and last run. A run gets its own directory with `report.md`, `findings.md`, and redacted `evidence/`. A result is appended the moment each case finishes, so nothing depends on memory, and an interrupted run resumes from the first pending case. Every bug becomes a test case. Both are committed: cases grow into the project's manual regression suite, and runs are the audit trail of what was tested at which commit.
